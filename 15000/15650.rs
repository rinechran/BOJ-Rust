...
fn permute<W: Write>(arr: &Vec<i32>, selected: &mut Vec<i32>, start : usize ,r: i32, output: &mut W) {
    ...
    for index in start..arr.len() {
        ...
        permute(arr, selected, index+1 ,r, output);
        selected.pop();
    }
}
...
fn main() {
    ...
    permute(&arr, &mut selected,0, k, &mut writer);
}
